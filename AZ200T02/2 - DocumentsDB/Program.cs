...
namespace AZ200T02._2
{
    class Program
    {
        ...
        private static async Task cosmosDBDemo()
        {
            ...
            FeedOptions DefaultOptions = null;
            var query = client.CreateDocumentQuery<Family>(collectionUri, new SqlQuerySpec() { QueryText = "SELECT * FROM f WHERE (f.surname = @lastName)", Parameters = new SqlParameterCollection() { new SqlParameter("@lastName", "Andt") } }, DefaultOptions);
